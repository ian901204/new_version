...
from ..utils.file_utils import FolderScanner
...
class FileUploaderApp:
    """Main application window for file upload tool."""
    ...
    def __init__(self, master):
        ...
        self.local_path = "./"
        self.folder_scanner: FolderScanner | None = None
        ...
        self.master.wait_window(self.master.winfo_children()[-1])
    ...
    def prompt_credentials(self):
        ...
        def submit_credentials():
            ...
            if auth_method_var.get() == "key" and self.server_handler.key_path.endswith('.pub'):
                messagebox.showerror("錯誤", "您選擇的是公鑰檔案（.pub）\n\n請選擇私鑰檔案（沒有 .pub 副檔名）")
                return
    ...
    def create_server_folder(self):
        ...
        if success:
            ...
            self.update_server_folders()
            self.server_folder_var.set(new_folder_name)

    def _get_all_folder_names(self) -> list:
        """Get all folder names from the scanner."""
        if self.folder_scanner is None:
            return []
        return self.folder_scanner.get_folder_names(use_cache=True)

    def calculate_pagination(self):
        total_items = len(self._get_all_folder_names())
    ...
    def get_current_page_items(self):
        all_folders = self._get_all_folder_names()
    ...
    def browse_local_folder(self):
        ...
        if self.local_path:
            ...
            # Initialize optimized folder scanner
            self.folder_scanner = FolderScanner(self.local_path)
            self.current_page = 0
            self.update_local_files()

    def refresh_local_folder(self):
        if self.folder_scanner is not None:
            self.folder_scanner.invalidate_cache()
            self.current_page = 0
            self.update_local_files()
    ...
    def show_local_relate_folder(self, name):
        """Search folders with optimized filtering."""
        if self.folder_scanner is None:
            return

        # Use the optimized filter method
        filtered_names = self.folder_scanner.filter_folders(name, use_cache=True)
        ...
        start_idx = self.current_page * self.page_size
        end_idx = start_idx + self.page_size
        for item in filtered_names[start_idx:end_idx]:
                ...
                with SCPClient(ssh.get_transport(), progress=self.update_progress) as scp:
                    ...
                    for index, item in enumerate(selected_items):
                        ...
                        stdin, stdout, stderr = ssh.exec_command(
                            f'test -e {self.server_handler.base_path}/{selected_server_folder}/{item} && echo "exists"'
                        )
